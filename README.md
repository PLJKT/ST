...
ST/
├── data/
│   ├── raw/          # 券商原始导出（POEMS xlsx + FUTU 成交/订单/资金明细 csv ×3）
...
- **v3（2026-09-23）**：新增三张图——总本金 vs 总盈亏（按年）、回本周期与条件测算、最大亏损/盈利 10 只个股对比（跨账户 USD 折算）。
- **v4（2026-09-23，按用户指令）**：FUTU 数据源**唯一化**——Excel 中的 FUTU 台账与月度矩阵（2021-22 旧账户口径）彻底剔除，一律以资金明细为准；成交/订单两 CSV 降级为逐笔互验。内置三方对账（仪表盘「数据对账」区）：541 笔成交 ↔ 资金明细全配对（±$0.02 结算舍入容差）；7 组同键重复成交行经资金侧同金额双记录裁决为真实分笔、非重复导入；订单费用 ↔ 资金费用差额恰等于融券使用费（USD 1,091.08 / HKD 22.86）；资金明细自身零重复、两币种余额链恒等式均闭合。
